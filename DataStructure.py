...
### 힙(Heap) ###
# 우선순위 큐를 구현하기 위해 사용하는 자료구조, 우선순위 큐는 우선순위가 가장 높은 데이터를 가장 먼저 삭제함.
# 따라서 데이터를 우선순위에 따라 처리하고 싶을 때 사용함. 우선순위 큐 라이브러리를 지원하기 때문에 코테에서
# 직접 힙 자료구조로 우선순위 큐를 구현하지 않아도 됨. heapq 라이브러리 사용.
# 내부적으로 최소 힙, 최대 힙을 이용하는데 최소 힙의 경우 우선순위가 낮은 데이터가 먼저 삭제되고
# 최대 힙의 경우 우선순위가 높은 데이터가 먼저 삭제된다. 